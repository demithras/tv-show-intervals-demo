...
WHERE category = 'Sports' AND 1 = (:query_type % 4) + 1;

-- Query 2: Channel filter (should use idx_programs_channel)
SELECT * FROM programs 
WHERE channel_id = :random_channel AND 2 = (:query_type % 4) + 1;

-- Query 3: Day filter (should use idx_programs_day)
SELECT * FROM programs 
WHERE day_of_week = :random_day AND 3 = (:query_type % 4) + 1;
