...
-- 10-minute intervals for TV programs
...
DROP FUNCTION IF EXISTS count_10min_intervals(TIME, TIME);
...
-- Program intervals table: stores calculated 10-minute intervals
...
-- Function to count 10-minute intervals
-- Handles overnight programs (crossing midnight)
-- Rules:
-- - If start == end → 0 intervals
-- - If duration < 10 minutes → 0 intervals  
-- - If duration is exactly N*10 minutes → N intervals
-- - Handles midnight crossing correctly
CREATE OR REPLACE FUNCTION count_10min_intervals(start_time TIME, end_time TIME)
RETURNS INTEGER AS $$
DECLARE
...
BEGIN
    ...
    -- Return number of complete 10-minute intervals
    -- If duration < 10 minutes, this returns 0
...
BEGIN
    ...
    ELSIF TG_OP = 'INSERT' THEN
        ...
        VALUES (NEW.program_name, count_10min_intervals(NEW.start_time, NEW.end_time))
        ON CONFLICT (program_name) 
        DO UPDATE SET 
            interval_count = count_10min_intervals(NEW.start_time, NEW.end_time),
...
COMMENT ON TABLE program_intervals IS 'Automatically calculated 10-minute intervals for each program';
COMMENT ON FUNCTION count_10min_intervals(TIME, TIME) IS 'Calculates number of 10-minute intervals, handling overnight programs';
